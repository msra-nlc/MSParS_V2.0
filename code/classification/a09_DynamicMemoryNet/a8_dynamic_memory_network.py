...
class DynamicMemoryNetwork:
    def __init__(self, num_classes, learning_rate, batch_size, decay_steps, decay_rate, sequence_length, story_length,
        ...
        self.answer_single = tf.placeholder(tf.int32, [None,num_classes], name="input_y")  # y:[None,num_classes]
        ...
        if not self.multi_label_flag:
            correct_prediction = tf.equal(tf.cast(self.predictions,tf.int32), tf.cast(tf.argmax(self.answer_single, axis=1),tf.int32)) #tf.argmax(self.logits, 1)-->[batch_size]
    ...
    def loss(self, l2_lambda=0.0001):  # 0.001
        with tf.name_scope("loss"):
            ...
            losses = tf.nn.softmax_cross_entropy_with_logits(labels=self.answer_single,logits=self.logits);  # sigmoid_cross_entropy_with_logits.#losses=tf.nn.softmax_cross_entropy_with_logits(labels=self.input_y,logits=self.logits)
