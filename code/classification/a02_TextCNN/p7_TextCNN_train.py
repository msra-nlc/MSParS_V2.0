        ...
        for epoch in range(curr_epoch,FLAGS.num_epochs):
            ...
            for start, end in zip(range(0, number_of_training_data, batch_size),range(batch_size, number_of_training_data, batch_size)):
                ...
                '''if start%(3000*FLAGS.batch_size)==0: # eval every 3000 steps.
                    eval_loss, f1_score,f1_micro,f1_macro,accuracy = do_eval(sess, textCNN, vaildX, vaildY,num_classes)
                    print("Epoch %d Validation Loss:%.3f\tF1 Score:%.3f\tF1_micro:%.3f\tF1_macro:%.3f\tAccuracy:%.3f" % (epoch, eval_loss, f1_score,f1_micro,f1_macro,accuracy))
                    
                    val_loss, f1_score,f1_micro,f1_macro,accuracy = do_eval(sess, textCNN, testX, testY,num_classes)
            ...
            if epoch % FLAGS.validate_every==0:
                predicate_y, eval_loss, f1_score,f1_micro,f1_macro,accuracy = do_eval(sess, textCNN, vaildX, vaildY,num_classes)
                print("Epoch %d Validation Loss:%.3f\tF1 Score:%.3f\tF1_micro:%.3f\tF1_macro:%.3f\tAccuracy:%.3f" % (epoch, eval_loss, f1_score,f1_micro,f1_macro,accuracy))
...
def do_eval(sess,textCNN,evalX,evalY,num_classes):  
    ...
    for start,end in zip(range(0,number_examples,batch_size),range(batch_size,number_examples,batch_size)):
        ...
        accuracy += right_or_not
        predicate_list = predicate_list + predict_y
    ...
    return predicate_list,eval_loss/float(eval_counter),f1_score,f1_micro,f1_macro,accuracy/len(evalX)
...
def compute_confuse_matrix(target_y,predict_y,label_dict,name='default'):
    ...
    for i in range(len(target_y)):
        if target_y[i] == predict_y[i]: accuracy += 1
    ...
    return label_dict, accuracy * 1.0 / len(target_y)
