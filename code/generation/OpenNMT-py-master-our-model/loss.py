...
def build_loss_compute(model, tgt_vocab, opt, train=True):
    ...
    if opt.copy_attn:
        ...
        compute1 = onmt.modules.CopyGeneratorLossCompute(
            model.generator1, tgt_vocab, opt.copy_attn_force,
            opt.copy_loss_by_seqlength)
        compute2 = onmt.modules.CopyGeneratorLossCompute(
            model.generator2, tgt_vocab, opt.copy_attn_force,
            opt.copy_loss_by_seqlength)
    else:
        compute = NMTLossCompute(
            model.generator, tgt_vocab,
            label_smoothing=opt.label_smoothing if train else 0.0)
        compute1 = NMTLossCompute(
            model.generator1, tgt_vocab,
            label_smoothing=opt.label_smoothing if train else 0.0)
        compute2 = NMTLossCompute(
            model.generator2, tgt_vocab,
            label_smoothing=opt.label_smoothing if train else 0.0)
    ...
    return compute,compute1, compute2
...
class LossComputeBase(nn.Module):
    ...
    def _make_shard_state(self, batch, tgt, output, range_, attns=None,alignment=None ,src_map=None):
        """
    ...
    def monolithic_compute_loss(self, batch, tgt, output, attns,alignment=None,src_map=None):
        """
        ...
        """
        range_ = (0, tgt.size(0))
        shard_state = self._make_shard_state(batch, tgt, output, range_, attns, alignment,src_map)
        _, batch_stats = self._compute_loss(batch,**shard_state)

        return batch_stats

    def sharded_compute_loss(self, batch, tgt, output, attns,
                             cur_trunc, trunc_size, shard_size,
                             normalization,alignment=None,src_map=None):
