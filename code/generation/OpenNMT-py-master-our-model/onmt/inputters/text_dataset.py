...
class TextDataset(DatasetBase):
    ...
    def __init__(self, fields, src_examples_iter, tgt_examples_iter,
        ...
        def filter_pred(example):
            """ ? """
            return 0 < len(example.src) <= 10000 \
                and 0 < len(example.tgt) <= 10000  and 0 < len(example.tgt1) <= 10000  and 0 < len(example.tgt2) <= 10000  and 0 < len(example.src1) <= 10000
    ...
    def make_examples(text_iter, truncate, side):
        ...
        if side == "tgt" or side == 'tgt1' or side == 'tgt2':
            for i, line in enumerate(text_iter):
                ...
                if truncate:
                    ...
                    entity1 = entity1[:truncate]
                    entity2 = entity2[:truncate]

                if "<e>" not in template:
                    template = "<e> or <e>".split()
                tgt1_index = template.index("<e>")
                tgt2_index = len(template) - 1 - template[::-1].index("<e>")

                words, feats, n_feats = \
                    TextDataset.extract_text_features(template)

                words1, feats1, n_feats1 = \
                    TextDataset.extract_text_features(entity1)

                words2, feats2, n_feats2 = \
                    TextDataset.extract_text_features(entity2)

                example_dict = {side: words, "tgt1": words1, "tgt2":words2,"tgt1_index": int(tgt1_index),
                            "tgt2_index": int(tgt2_index), "indices": i}
            ...
            for i, line in enumerate(text_iter):
                ...
                if truncate:
                    ...
                    entity1 = " ".join(line[1].split()[:truncate])
                    entity2 = " ".join(line[2].split()[:truncate])
                entity = " <SP> ".join([entity1, entity2]).split()

                words, feats, n_feats = \
                    TextDataset.extract_text_features(context)

                words1, feats1, n_feats1 = \
                    TextDataset.extract_text_features(entity)

                example_dict = {side: words, "src1":words1, "indices": i}
    ...
    def get_fields(n_src_features, n_tgt_features):
        ...
        fields["tgt1_index"] = torchtext.data.Field(sequential=False, use_vocab=False)
        fields["tgt2_index"] = torchtext.data.Field(sequential=False, use_vocab=False)
        ...
        fields["src1_map"] = torchtext.data.Field(
            use_vocab=False, dtype=torch.float,
            postprocessing=make_src, sequential=False)
        ...
        fields["alignment1"] = torchtext.data.Field(
            use_vocab=False, dtype=torch.long,
            postprocessing=make_tgt, sequential=False)

        fields["alignment2"] = torchtext.data.Field(
            use_vocab=False, dtype=torch.long,
            postprocessing=make_tgt, sequential=False)
    ...
    def _dynamic_dict(self, examples_iter):
        for example in examples_iter:
            ...
            src1 = example["src1"]
            src_vocab1 = torchtext.vocab.Vocab(Counter(src1),
                                              specials=[UNK_WORD, PAD_WORD])
            self.src_vocabs1.append(src_vocab1)
            # Mapping source tokens to indices in the dynamic dict.
            src1_map = torch.LongTensor([src_vocab1.stoi[w] for w in src1])
            ...
            if "tgt1" in example:
                ...
                mask1 = torch.LongTensor(
                    [0] + [src_vocab1.stoi[w] for w in tgt1] + [0])
                example["alignment1"] = mask1

            if "tgt2" in example:
                tgt2 = example["tgt2"]
                mask2 = torch.LongTensor(
                    [0] + [src_vocab1.stoi[w] for w in tgt2] + [0])
...
class ShardedTextCorpusIterator(object):
    ...
    def _example_dict_iter(self, line, index):
        if self.side == "tgt" or self.side == "tgt1" or self.side == "tgt2":
            ...
            if self.line_truncate:
                template = template[:self.line_truncate]
                entity1 = entity1[:self.line_truncate]
                entity2 = entity2[:self.line_truncate]

            if "<e>" not in template:
                template = "<e> or <e>".split()
            tgt1_index = line[0].index("<e>")
            tgt2_index = len(template) - 1 - template[::-1].index("<e>")
            ...
            example_dict = {self.side: words, "tgt1": words1, "tgt2": words2, "tgt1_index": int(tgt1_index),
                            "tgt2_index": int(tgt2_index), "indices": index}
            ...
            if self.line_truncate:
                context = context[:self.line_truncate]
                entity1 = " ".join(line[1].split()[:self.line_truncate])
                entity2 = " ".join(line[2].split()[:self.line_truncate])
            entity = " <SP> ".join([entity1,entity2]).split()
            ...
            example_dict = {self.side: words, "src1": words1, "indices": index}
