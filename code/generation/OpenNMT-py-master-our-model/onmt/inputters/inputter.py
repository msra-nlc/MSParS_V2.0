...
def build_vocab(train_dataset_files, fields, data_type, share_vocab,
        ...
        for ex in dataset.examples:
            for k in fields:
                ...
                if not fields[k].sequential:
                    ...
                    continue
                elif k == 'src1' and src_vocab:
                    continue
                elif k == 'tgt1' and tgt_vocab:
                    continue

                elif k == 'tgt2' and tgt_vocab:
                    continue
    ...
    _build_field_vocab(fields["tgt1"], counter["tgt1"],
                       max_size=tgt_vocab_size,
                       min_freq=tgt_words_min_frequency)
    _build_field_vocab(fields["tgt2"], counter["tgt2"],
                       max_size=tgt_vocab_size,
                       min_freq=tgt_words_min_frequency)
    logger.info(" * tgt vocab size: %d." % len(fields["tgt"].vocab))
    logger.info(" * tgt1 vocab size: %d." % len(fields["tgt1"].vocab))
    ...
    if data_type == 'text':
        ...
        _build_field_vocab(fields["src1"], counter["src1"],
                           max_size=src_vocab_size,
                           min_freq=src_words_min_frequency)
        logger.info(" * src1 vocab size: %d." % len(fields["src1"].vocab))
        ...
        if share_vocab:
            ...
            merged_vocab = merge_vocabs(
                [merged_vocab, fields["src1"].vocab],
                vocab_size=src_vocab_size,
                min_frequency=src_words_min_frequency)

            merged_vocab = merge_vocabs(
                [merged_vocab, fields["tgt1"].vocab],
                vocab_size=src_vocab_size,
                min_frequency=src_words_min_frequency)

            merged_vocab = merge_vocabs(
                [merged_vocab, fields["tgt2"].vocab],
                vocab_size=src_vocab_size,
                min_frequency=src_words_min_frequency)
            fields["src"].vocab = merged_vocab
            fields["tgt"].vocab = merged_vocab
            fields["src1"].vocab = merged_vocab
            fields["tgt1"].vocab = merged_vocab
...
def build_dataset_iter(datasets, fields, opt, is_train=True):
    ...
    if is_train and opt.batch_type == "tokens":
        def batch_size_fn(new, count, sofar):
            ...
            max_src_in_batch = max(max_src_in_batch, len(new.src1) + 2)
            # Tgt: w1 ... wN <eos>
            max_tgt_in_batch = max(max_tgt_in_batch, len(new.tgt) + 1)
            max_tgt_in_batch = max(max_tgt_in_batch, len(new.tgt1) + 1)
