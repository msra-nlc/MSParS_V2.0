...
def build_base_model(model_opt, fields, gpu, checkpoint=None):
    ...
    if model_opt.model_type == "text":
        ...
        feature_dicts1 = inputters.collect_feature_vocabs(fields, 'src1')
        src_embeddings = build_embeddings(model_opt, src_dict, feature_dicts)
        src_embeddings1 = build_embeddings(model_opt, fields["src1"].vocab, feature_dicts1)
        encoder = build_encoder(model_opt, src_embeddings)


        encoder1 = build_encoder(model_opt, src_embeddings1)
    ...
    tgt_dict1 = fields["tgt1"].vocab
    feature_dicts1 = inputters.collect_feature_vocabs(fields, 'tgt1')
    tgt_embeddings1 = build_embeddings(model_opt, tgt_dict1,
                                      feature_dicts1, for_encoder=False)
    ...
    decoder2 = build_decoder(model_opt, tgt_embeddings1)

    # Build NMTModel(= encoder + decoder).
    device = torch.device("cuda" if gpu else "cpu")
    model = onmt.models.NMTModel(encoder,encoder1, decoder, decoder1, decoder2)

    # Build Generator.
    if not model_opt.copy_attn:
        if model_opt.generator_function == "sparsemax":
            gen_func = onmt.modules.sparse_activations.LogSparsemax(dim=-1)
            gen_func1 = onmt.modules.sparse_activations.LogSparsemax(dim=-1)
            gen_func2 = onmt.modules.sparse_activations.LogSparsemax(dim=-1)
        else:
            gen_func = nn.LogSoftmax(dim=-1)
            gen_func1 = nn.LogSoftmax(dim=-1)
            gen_func2 = nn.LogSoftmax(dim=-1)
        generator = nn.Sequential(
            nn.Linear(model_opt.dec_rnn_size, len(fields["tgt"].vocab)),
            gen_func
        )
        generator1 = nn.Sequential(
            nn.Linear(model_opt.dec_rnn_size, len(fields["tgt1"].vocab)),
            gen_func1
        )
        ...
        generator = CopyGenerator(model_opt.dec_rnn_size,
                                  fields["tgt"].vocab)
        generator1 = CopyGenerator(model_opt.dec_rnn_size,
                                  fields["tgt1"].vocab)
